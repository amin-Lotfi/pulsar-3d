...
void *ProcGetImage(void *pParam)
{
    ...
    emStatus = GXGetIntValue(g_hDevice, "Width", &stWidthValue);
    nWidth = stWidthValue.nCurValue;
    if (emStatus != GX_STATUS_SUCCESS)
    {
        printf("Get camera info failed, please restart this app!\n");
        return 0;
    }
    emStatus = GXGetIntValue(g_hDevice, "Height", &stHeightValue);
    nHeight = stHeightValue.nCurValue;
    if (emStatus != GX_STATUS_SUCCESS)
    {
        printf("Get camera info failed, please restart this app!\n");
        return 0;
